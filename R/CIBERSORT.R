...
# Requirements:
# R v3.0 or later. (dependencies below might not work properly with
#    earlier versions)
# install.packages('e1071')
# install.pacakges('parallel')
# install.packages('preprocessCore')
# if preprocessCore is not available in the repositories you have selected,
# run the following:
#           source("http://bioconductor.org/biocLite.R")
#           biocLite("preprocessCore")
# Windows users using the R GUI may need to Run as Administrator to
# install or update packages.
# This script uses 3 parallel processes.  Since Windows does not support
# forking, this script will run
...
# not be represented as fractions (to derive the default output, normalize
# absolute levels such that they sum to 1 for each mixture sample)
#     - the sum of all cell subsets in each mixture sample will be added to the
#       ouput
#       ('Absolute score'). If LM22 is used, this score will capture total
#     immune content.
# iv)  abs_method = if absolute is set to TRUE, choose method: 'no.sumto1' or
#  'sig.score'
...
# Output: matrix object containing all results and tabular data written to
# disk 'CIBERSORT-Results.txt'
